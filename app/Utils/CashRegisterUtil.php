...
class CashRegisterUtil extends Util
{
    ...
    public function countOpenedRegister()
    {
        ...
        if (Config::get('app.caja_unica')) {
            $count =  CashRegister::where('business_id', $user->business_id)
                ->where('status', 'open')
                ->count();
        } else {
            $count =  CashRegister::where('user_id', $user->id)
                ->where('status', 'open')
                ->count();
        }
    ...
    public function addSellPayments($transaction, $payments)
    {
        ...
        if (Config::get('app.caja_unica')) {
            $register =  CashRegister::where('business_id', $user->business_id)
                ->where('status', 'open')
                ->first();
        } else {
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        if (empty($register_id)) {
            ...
            if (Config::get('app.caja_unica')) {
                $query->where('cash_registers.business_id', $user->business_id)
                    ->where('cash_registers.status', 'open');
            } else {
                $query->where('cash_registers.user_id', $user->id)
                    ->where('cash_registers.status', 'open');
            }
        } else {
