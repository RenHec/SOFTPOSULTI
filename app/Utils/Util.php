...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        if (!empty($location)) {
            $seleccionados = Config::get('constants.payment_types');
            $payment_types = array();

            foreach ($seleccionados as $key => $value) {
                if ($seleccionados[$key]['mostrar']) {
                    $payment_types = [$seleccionados[$key]['lang'] => __('lang_v1.' . $seleccionados[$key]['lang'])] + $payment_types;
                }
            }
